import type { ProviderConfig, ProbeResult, ProviderId } from './types';
...
export function detectProvider(key: string): ProviderConfig | null {
  // Longest prefix wins — anthropic (sk-ant-) before openai (sk-)
  const sorted = [...PROVIDERS].sort(
    (a, b) => Math.max(...b.prefix.map((p) => p.length)) - Math.max(...a.prefix.map((p) => p.length)),
  );
  for (const p of sorted) {
    if (p.prefix.some((pfx) => key.startsWith(pfx))) return p;
  }
  return null;
}
